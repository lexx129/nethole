...
init_port_array (void)
{
  ...
  if (ctl.mode & FL_RESTRICT_PORTS) {
    ...
    for (ip = 0; ip < ctl.addresses + 2; ip++) {
      /* Set only PORT_AMOUNT ports to be monitored */
      for (i = 0; i <= PORT_USR_AMOUNT; i++) {
        int random_num = rand_uint16(ctl.rand);

        /* Skip system ports */
        if (random_num < PORT_SYSTEM_MAX)
          ctl.ips_port_array[ip].usr_port[i] = random_num + PORT_SYSTEM_MAX;
        else
          ctl.ips_port_array[ip].usr_port[i] = random_num;

        DEBUG_PRT(DEBUG_PKTPROC, "random nums ip %d, rand 1 %d", ip, ctl.ips_port_array[ip].usr_port[i]);
      }

      for (i = 0; i <= PORT_SYS_AMOUNT; i++) {
        int random_num = rand_uint16(ctl.rand);

        ctl.ips_port_array[ip].sys_port[i] = random_num % (sizeof(tgt_ports)/sizeof(long));
        DEBUG_PRT(DEBUG_PKTPROC, "random nums ip %d, rand 2 %d", ip, ctl.ips_port_array[ip].sys_port[i]);
      }
...
ctl_init()
{
  ...
  if (ctl.mode & FL_RAND_MAC) {
    ctl.rand = rand_open();
    if ((ctl.mac_array = calloc(ctl.addresses+2, sizeof(*ctl.mac_array))) == NULL) {
      warnx(  "*** calloc error: mac_array - unable to allocate memory" );
      util_clean_exit(1);
    }
    int ip;
    for (ip = 0; ip < ctl.addresses + 2; ip++) {
  ...
  /* Allocate port array in port restrict mode (-S) */
  if (ctl.mode & FL_RESTRICT_PORTS) {
    if (NULL == ctl.rand) {
      ctl.rand = rand_open();
    }
    if ((ctl.ips_port_array = calloc(ctl.addresses+2, sizeof(*ctl.ips_port_array))) == NULL) {
      warnx(  "*** calloc error: ips_port_array - unable to allocate memory" );
      util_clean_exit(1);
    }
  }
